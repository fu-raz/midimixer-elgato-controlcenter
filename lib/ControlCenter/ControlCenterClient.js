...
module.exports = function(settings) {
    ...
    this.initConnection = function(tryAgain, _cb) {
        ...
        try {
            ...
            websocket.onclose = function(evt) {
                console.warn(`[cclient::websocket.onclose] WEBSOCKET CLOSED -> reason:`, evt);
                self.clearTimers();
                self.initialized = false;
                if(tryAgain === true) {
                    checkTimeout = setTimeout(() => self.checkConnection(false), 2500);
                }
                events.emit('deviceConfigurationChanged', null);
            };

            websocket.onerror = function(evt) {
                console.warn(`[ccclient] websocket.onerror`, evt);
                self.initialized = false;
    ...
    this.initCCRPC = function() {
        ...
        jrpc.toStream = function(msg) {
            debugLog('[ccc_client::toStream] sending', msg);
            try {
                websocket.send(msg);
            } catch(error) {
        ...
        jrpc.on('deviceAdded', ['deviceID', 'type', 'name'], (deviceID, type, name) => {
            console.log('[jsonrpc] deviceAdded', deviceID, type, name);
            ...
            events.emit('deviceAdded', {deviceID: deviceID, type: type, name: name});
        });

        jrpc.on('deviceRenamed', ['deviceID', 'name', 'type'], (deviceID, name, type) => {
            debugLog('[jsonrpc] deviceRenamed', deviceID, name, type);
            self.setDeviceProperty(deviceID, 'name', name);
            events.emit('deviceRenamed', {deviceID: deviceID, name: name});
        });

        jrpc.on('deviceRemoved', ['deviceID', 'type', 'name'], (deviceID, type, name) => {
            ...
            events.emit('deviceRemoved', {deviceID: deviceID, name: name});
        });

        jrpc.on('deviceConfigurationChanged', ['deviceID', 'lights', 'battery', 'favoriteColors', 'favoriteScenes'], (deviceID, lights, battery, favoriteColors, favoriteScenes) => {
            debugLog('[jsonrpc] deviceConfigurationChanged', deviceID, lights, 'BATTERY', battery, 'favoriteColors', favoriteColors, 'favoriteScenes', favoriteScenes);
        ...
        jrpc.call('getApplicationInfo').then(function(result) {
            // console.log('%c%s', clr, '[jsonrpc] getApplicationInfo', result);
            // console.log("%c%s", clr, '[jsonrpc] connected');
            // console.log('self.propertyInspectorShown', self.propertyInspectorShown);
    ...
    this.getDevices = function() {
        // console.log('%c%s', 'color: black; background: yellow; font-size: 11px;', '[jsonrpc] getDevices -------');
    ...
    this.getDeviceConfiguration = function(forDevice, callback) {
        debugLog('getDeviceConfiguration 1', forDevice);
        jrpc.call('getDeviceConfiguration', {deviceID: forDevice['deviceID']}).then(function(data) {
            forDevice['lights'] = data.lights || {};
            ['battery', 'favoriteColors', 'favoriteScenes'].forEach(e => {
                if(data.hasOwnProperty(e) && data[e] !== null) {
                    forDevice[e] = data[e];
                }
            });
            // merge new values with cached device
            Object.entries(data).map(e => (forDevice[e[0]] = e[1]));
    ...
    this.setDeviceConfiguration = function(forDevice, prop) {
        debugLog('[jsonrpc] setDeviceConfiguration for device (orig):', forDevice['deviceID'], ` property:${prop}`, forDevice['lights']);
        ...
        // console.log('setDeviceConfiguration 2', device);
        // jrpc.call('setDeviceConfiguration',{'deviceID': forDevice['deviceID'] ,'lights': {'brightness': x}}).then(function(result){
        // jrpc.call('setDeviceConfiguration', { 'deviceID': forDevice['deviceID'], 'lights': lights }).then(function (result) {
        jrpc.call('setDeviceConfiguration', device).then(function(result) {
            console.assert(!result, '[jsonrpc] setDeviceConfiguration -> result:', result);
        });
    };
};